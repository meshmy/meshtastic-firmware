...
{
    return cellState == CELL_BEARER_UP && cellLocalIP.length() > 0;
}
...
static void setCellState(CellState next)
{
    ...
    if (next != CELL_REG_WAIT && next != CELL_BEARER_UP)
        cellDiagInvalidate();
...
static int32_t reconnectCell()
{
    ...
    bool silent = (cellState == CELL_SIM_WAIT || cellState == CELL_REG_WAIT || cellState == CELL_BEARER_UP) &&
    ...
    case CELL_BEARER_UP:
        if (cellLocalIP.length() == 0)
            stepBearer();
        else if (!cellRtcSeeded) {
            cellBusy = true;
